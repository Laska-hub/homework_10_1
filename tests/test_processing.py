...
def test_filter_by_state_default(sample_transactions: List[Transaction]) -> None:
    result = filter_by_state(sample_transactions)
    assert len(result) == 2
    assert all(tx["state"] == "EXECUTED" for tx in result)


def test_filter_by_state_custom(sample_transactions: List[Transaction]) -> None:
    result = filter_by_state(sample_transactions, state="CANCELED")
    assert len(result) == 1
    assert result[0]["state"] == "CANCELED"


def test_sort_by_date_descending(sample_transactions: List[Transaction]) -> None:
    result = sort_by_date(sample_transactions)
    dates = [tx["date"] for tx in result]
    assert dates == sorted(dates, reverse=True)


def test_sort_by_date_ascending(sample_transactions: List[Transaction]) -> None:
